...
from convert_toggl_report import two_d_array_to_csv, convert_toggl_report_to_python_array, \
    extract_time_code_from_string, extract_activity_name_from_string, extract_absence_type_from_string

the_output = """,,lege,Vaksinering,0,0,0,0,2.5
32010000-7,,,Bandrigging til Halloween-fest,0,0,0,0,4.5
...
class TestConvertTogglReport(TestCase):
    ...
    def test_extract_absence_type_from_string(self):
        self.assertEqual("Deltakelse", extract_absence_type_from_string("foo bar fravær:Deltakelse"))
        self.assertEqual("Ledelse/koordinering",
                         extract_absence_type_from_string("foo bar fravær:Ledelse/koordinering"))
        self.assertEqual("Ledelse/koordinering", extract_absence_type_from_string("fravær:Ledelse/koordinering"))
        self.assertEqual("Denne strengen har mellomrom",
                         extract_absence_type_from_string("fravær:Denne strengen har mellomrom"))
        self.assertEqual(None, extract_absence_type_from_string("Denne strengen har ingen fravær"))
